...
export const registerFormSchema = z
  .object({
    ...
    email: z
      .string()
      .nonempty("Oops, email is missing!")
      .email("Enter a valid email"),

    password: z
      .string()
      .nonempty("Password is a must!")
      .min(6, "Password")
      .regex(
        /(?=.*?[A-Z])/,
        "Add an uppercase letter, it’s like a VIP in your password!"
      )
      .regex(/(?=.*?[a-z])/, "Don’t be shy, throw in a lowercase letter!")
      .regex(/(?=.*?[0-9])/, "Numbers make passwords smarter! Add one.")
      .regex(
        /(?=.*?[!@#$%^&*(),.?":{}|<>])/,
        "Special characters add spice! Include one."
      ),
